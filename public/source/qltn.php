...
//echo $id_user."<br>";
$sql_thongtin="select * from tbthanhvien where iduser='$id_user'";
$data_thongtin=$o->query($sql_thongtin);
$arrThongtin=$data_thongtin->fetchAll(PDO::FETCH_ASSOC);
//print_r($arrThongtin);
//
$sql_hdon="select * FROM tbhoadon WHERE iduser='$id_user' ORDER BY idhoadon DESC";
						...
						<div>
							...
							<table width="100%" border="0px">
								<tbody>
									...
									<tr>
										<!--donhang-->
										<td><?php echo $v['idhoadon'];
										$id_hoadonct=$v['idhoadon'];
										//echo $id_hoadonct;
										?></td>
										<td><?php echo $v['ngaydat'];?></td>
										<?php
											$sql_cthdon="select chitietgoidoan.DONGIA, thucankem.HINHANH FROM chitietgoidoan INNER JOIN thucankem ON chitietgoidoan.MATHUCAN = thucankem.MATHUCAN where idhoadon ='$id_hoadonct'";
										...
										?>
										<img height="50px" width="50px" src="img/<?php echo $v['HINHANH'];?>" />
										<?php $sumgia+=$v['DONGIA']; }?>
										</td>
										<td><?php echo dinhdangso($sumgia);?> ₫</td>
									
										<td><a href="qldonhang.php?idhoadon=<?php echo $id_hoadonct;?>">Quản lý</a></td>
										<!--/donhang-->
									</tr>
									<?php
										$i++;
